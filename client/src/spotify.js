//ensure to check the last commented items
...
const getAccessToken = () => {
    ...
    const queryParams = {
        //get method{get()} takes the key pair in the URL as parameter and returns the value.
      [LOCALSTORAGE_KEYS.accessToken]: urlParams.get('access_token'),
      [LOCALSTORAGE_KEYS.refreshToken]: urlParams.get('refresh_token'),
      [LOCALSTORAGE_KEYS.expireTime]: urlParams.get('expires_in'),
    };
    const hasError = urlParams.get('error'); //from index.js
    ...
    if (queryParams[LOCALSTORAGE_KEYS.accessToken]) {
      ...
      for (const property in queryParams) {
        window.localStorage.setItem(property, queryParams[property]); //store keys & their values.
      }
      // Set timestamp
      window.localStorage.setItem(LOCALSTORAGE_KEYS.timestamp, Date.now());
